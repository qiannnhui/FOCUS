...
def load_mvtec_json(json_file, image_dir, gt_dir, semseg_dir, meta):
...
def register_mvtec_dataset(name, metadata, image_root, panoptic_json, instances_json=None, panoptic_root=None, semantic_root=None):
    """
    註冊 MVTEC 合併後的資料集：
    - `mvtec_train`
    - `mvtec_val`
...
_PREDEFINED_SPLITS_MVTEC = {
    "mvtec2_train": (
        ...
        "MVTEC2/mvtec2_train_INSTANCE.json",  # `train` 仍然需要 Instance JSON（即便 annotations 為空）
    ),
    "mvtec2_val": (
        "MVTEC2",  # `test` 圖片
        "MVTEC2",  # `test` ground truth
        "MVTEC2/mvtec2_val_SEMANTIC.json",  # `test` semantic segmentation
        "MVTEC2/mvtec2_val_INSTANCE.json",  # `test` instance segmentation
    ),
}
