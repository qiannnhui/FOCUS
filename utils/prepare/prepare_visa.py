...
def create_instance_dataset(dataset_dir, output_dir):
    """
    ...
    """
    image_dir = os.path.join(dataset_dir, f"images")
    mask_dir = os.path.join(dataset_dir, "ground_truth_mask")

    image_files = sorted(glob.glob(os.path.join(image_dir, "*.JPG")))
    mask_files = sorted([os.path.join(mask_dir, os.path.splitext(os.path.basename(f))[0] + ".jpg") for f in image_files])
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"VISA_INSTANCE.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
    print(f"VISA in COCO format is saved to {output_file_path}")
def create_semantic_dataset(dataset_dir, output_dir):
    ...
    # Save to file
    output_file_path = os.path.join(output_dir, f"VISA_SEMANTIC.json")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    with open(output_file_path, 'w') as f:
        json.dump(coco_format, f)
...
if __name__ == "__main__":
    dataset_dir = "datasets/VISA"  # Update this to the path of your dataset
    output_dir = "datasets/VISA"  # Update this to your desired output directory
    
    create_instance_dataset(dataset_dir, output_dir)
    create_semantic_dataset(dataset_dir, output_dir)

    for name in ["ground_truth_mask"]:
